...
#define PESSOA_H 50
#define PESSOA_W 30
#define POLICIAL_VEL 3
#define BURGLAR_VEL 0.5
#define PISO_H 30
#define NUM_SALAS 3
...
void atualizaPolicial(Pessoa *cop, Mud obst[NUM_SALAS][NUM_ANDARES]){
	...
	if(cop->x_dir) {
     ...
     // Aplica gravidade
    if (cop->pulo) {
        cop->y_vel += GRAVIDADE; // Incrementa velocidade vertical devido à gravidade
        cop->y += cop->y_vel;   // Atualiza posição vertical
    }

    // Checa se o policial toca o chão
    if (cop->y >= cop->andar * (-SCREEN_H / NUM_ANDARES) + SCREEN_H - PISO_H) {
        cop->y = cop->andar * (-SCREEN_H / NUM_ANDARES) + SCREEN_H - PISO_H; // Alinha ao chão
        cop->y_vel = 0;        // Para o movimento vertical
        cop->pulo = 0;         // Reseta estado de pulo
        cop->pulos_restantes = 2; // Restaura os 2 pulos disponíveis
    }
	...
	if (cop->x >= obst[cop->sala][cop->andarMud].x && 
		...
		cop->y == SCREEN_H - PISO_H - (cop->andar*SCREEN_H/NUM_ANDARES)){
			...
			cop->vel = 0.5;
			cop->pulos_restantes = 0;
	}
	else{
		cop->vel = POLICIAL_VEL;
	}
...
void teletransporte(Pessoa *p){
	p->x = PESSOA_W + rand()%500;
    p->x_mall = p->x;

	int x = rand()%3;
	
	p->y = SCREEN_H - PISO_H - x*(SCREEN_H/NUM_ANDARES);
	p->andar = x;
	p->andarMud = NUM_ANDARES - x - 1;
...
int main(int argc, char **argv){
   ...
   //Musica
	...
	if (!musica) {
		fprintf(stderr, "Erro ao carregar a música 'm1.ogg'. Certifique-se de que o arquivo está no diretório correto.\n");
		return -1;
	}
		...
		//se o tipo de evento for um pressionar de uma tecla
			...
			switch(ev.keyboard.keycode) {
				...
				case ALLEGRO_KEY_SPACE:
					if (cop.pulos_restantes > 0) { // Permite pulo
                        cop.pulo = 1;              // Ativa estado de pulo
                        cop.y_vel = -JUMP_VEL;     // Velocidade inicial para subir
                        cop.pulos_restantes--;     // Reduz os pulos restantes
                    }
                    break;
				case ALLEGRO_KEY_K:
					teletransporte(&cop);
